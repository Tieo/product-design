...
class AI:
    ...
    def __init__(self): # ai 초기화, 지워도 될 듯 일단 보류 
        self.name = 'playerai'
        self.money = 50000
        self.fold = 1
        self.result = 0
    ...
    def situation(self, fold, turn, comcard,pot, table): #테이블 정보 업데이트 
        now = [fold, turn, comcard,pot, table]
        return now

    def is_now(self, players): #ai의 턴인지 확인 
        who = [players]
        return who 
    
    def action(self, players, table): #ai의 턴인 경우 랜덤으로 1,2,3 중 action을 선택함. 
        ...
        print("in 'action' ai's pick: ", act)
        return act

    def bet(self, players, act): #RAISE 한 경우 금액 결정 
        if players == 'playerai' and act == 3:
            bet = random.randrange(400, 2001)
        else:
            bet = None
        return bet

    def showhand(self): # FOLD 한 경우 AI 가 패 공개에 대해 결정 
        showhand = random.randrange(1,5)
        return showhand

    def status(self, name, money, fold, result, hand): # ai의 상태 정보 업데이트 
        if name == 'playerai':
            nowstatus = [name, money, fold, result, hand]
        else:
            nowstatus = None
        return nowstatus


#판에 깔린 정보를 리스트로 받아서 넣는다. 

ai = AI()
print(type(ai))
print(ai.name)
#print(ai.read_now(fold, turn, comcard, pot, table))
#print('플레이어들:', players)
    ...
    while 1:
        ...
        while 1:
            if players[i][2] == 0:
                ...
                break
            #@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@턴이 진행되는 부분 @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
            print("What's going on?", ai.situation(fold, turn, comcard,pot, table))
            print("who's turn is it now?", ai.is_now(players[i][0])) # players[i][0] 현재 플레이어 
            #print("AI STATUS: ", ai.status(player[i][0], player[i][1], player[i][2], player[i][3], player[i][4]))
            #print("AI STATUS: ", ai.money) #일단 보류

            if players[i][0] == 'playerai': 
                act = ai.action(players[i][0], table)
                print("ai's act ", act)
                bet = ai.bet(players[i][0], act)
                print("ai's bet: ", bet)

            elif table[i] != max(table) and max(table) != 2000: #if를 elif로 수정, @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
                act = int(input('%s의 턴입니다.\n행동을 입력하세요.\n1 : Fold\n2 : Call\n3 : Raise\n0 : 내상태보기\n' %players[i][0]))
            elif table[i] == max(table) and max(table) != 2000:
                act = int(input('%s의 턴입니다.\n행동을 입력하세요.\n1 : Fold\n2 : Check\n3 : Raise\n0 : 내상태보기\n' %players[i][0]))
            elif table[i] != max(table) and max(table) == 2000:
            ...
            if act == 1:
                ...
                if players[i][0] == 'playerai': # 추가한 부분 @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
                    bet = ai.bet(players[i][0],act)
                    players[i][1] -= bet
                    table[i] += bet
                    calls = 1
                ...
                elif lowlimit < 2000: #ai 플레이어 분기 추가
                    ...
                    if lowlimit<=bet<=2000:
                        ...
                        print('다시 입력해 주십시오')
                        continue
                elif 2000<=lowlimit: # 무슨 상황인 부분인지? 다시 입력해야하는데 그냥 다음 턴으로 넘어가짐 
...
while 1:
    opnum = int(input('상대의 숫자?(-1 : 게임종료, 0: AI와 PLAYER 대결, 최대 4)'))
    if opnum == -1:
        sys.exit()
    elif 0 <= opnum < 5:
        break ##상대의 수 정하기

playerai = ['playerai', 50000, 1, 0]
player = ['player', 50000, 1, 0]##플레이어 초기화, 50000은 자본금, 1은 폴드가 아님을 표시, 0은 승패용 점수
players = [player, playerai]
...
while 1:
    ...
    while 1:
        ...
        betting()
        tabletopot()
        if fold == opnum+1:##ai와 player 빼고 모두가 폴드한 경우 @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
            for i in range(len(players)):
                if players[i][2] == 1:
                    tabletopot()
                    players[i][1] += pot
                    pot = 0
                    if players[i][0] == 'playerai':
                        showhand = ai.showhand()
                        print("ai's showhand", showhand)
                    else:
                        #ai 분기 추가를 위해 수정@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
                        showhand = int(input('%s님, 패를 공개하시겠습니까?\n1. 전부 공개\n2.%s만 공개\n3.%s만 공개\n그 외 키 : 공개안함\n' %(players[i][0],players[i][4], players[i][5])))
                    if showhand == 1:
