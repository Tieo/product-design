...
def balancecheck():##5.13추가 
    for i in range(len(players)):
        if players[i][1] < 2000:
            if i == 0:
                print('잔액이 부족하여 게임을 진행할 수 없습니다. 패배하였습니다.\n')
                sys.exit()
            else:
                print('opponent%s는 잔액이 부족하여 패배하였습니다.\n' %i)
                del players[i]
    ...
    def turnaction(self, turn, aipt, someonebet, hand1, hand2): #턴 별 액션 
        ...
        if turn == 0: #turn0 일때 핸드체크
            if self.hand_check(hand1, hand2) == 'premium' or self.hand_check(hand1, hand2) == 'special' or self.hand_check(hand1, hand2) == 'good':
                ...
                #@@@@@@@@@@@@@@30퍼센트의 확률로 콜
                a = random.choices(range(1,3,1), weights=[0.3, 0.6]) # 0 30% 1 60%
                if a == 0:
                    return 2
                return 1
            ...
            if someonebet:
                if mybt > someonebet[-1]:
                    ...
                    if 2000 in someonebet and aipt >= 3:
                        return 2
                    #@@@@@@@@@@@@30퍼센트의 학률로 콜
                    if  random.choices(range(1,3,1), weights=[0.3, 0.6]) == 0:
                         return 2
                    return 1 
